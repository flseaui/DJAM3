...
namespace Anonym.Util
{
    public static class SerializedPropertyEX
    {
		...
		public static object GetParentOfCurrent(this SerializedProperty property, int iParentDepth = 0) {
			var binding_flags = System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic;
			...
			var property_names = property.propertyPath.Replace(".Array.data", ".").Split('.');
            int property_names_count = property_names.Count();

            for (int i = 0; i < property_names_count; ++i)
            {
                results.Add(result);
                if (i == property_names_count - 1)
                    break;
				...
				if (-1 == indexer_start)
                {
                    lastFieldInfo = parent.GetType().GetField(property_name, binding_flags);
                    result = lastFieldInfo.GetValue(parent);
				}
                else
                {
                    var array = parent as IEnumerable;
                    var index = Convert.ToInt32(new string(property_name.Where(c => char.IsDigit(c)).ToArray()));
            ...
            return results[results.Count - 1 - iParentDepth];
		}
